...
~~**P-INPUT-01**~~ RESOLVED — `from_dict()` now coerces types; `validate()` uses
`_check_range()` with TypeError guard.

~~**P-XSS-01**~~ RESOLVED — `esc()` helper added; all innerHTML interpolations of
server data now escaped (gates, stages, nominal comparison, scenario summary).

~~**P-TEST-01**~~ RESOLVED — `TestCircularizeBoundary` covers v_vert=49/50/51/−49/−51.

~~**P-TEST-02**~~ RESOLVED — `TestCLIScenarioFlag` covers argparser for both
`server.py --scenario` and `ascent_sim.py --compare`.

~~**P-TEST-03**~~ RESOLVED — `TestAPIErrorPaths` covers all 400 paths for
start/pause/resume/reset/speed without ScriptedTelemetry, out-of-range speed,
unknown preset, invalid body.

~~**P-TEST-04**~~ PARTIALLY RESOLVED — `wait_for()` polling helper added and applied
to the most flaky tests (1.0s sleeps). Remaining short sleeps (0.1-0.3s) are
acceptably fast; full conversion deferred.

~~**P-FUEL-01**~~ RESOLVED (v1.1.0) — stage dV bars use `dv_vac / dv_initial`.

~~**P-FUEL-02**~~ RESOLVED — preset loads now accept `noise_pct` and `playback_speed`
overrides from the request body.

~~**P-TELEM-01**~~ RESOLVED — `time_to_ap` set to `None` in offline modes (UI shows "—")
instead of misleading countdown.

~~**P-TELEM-02**~~ RESOLVED — g_force now returns 0 during coast (freefall) and
...
~~**P-TELEM-05**~~ RESOLVED — periapsis now gets `noise()` in both offline modes.

~~**P-MEM-01**~~ RESOLVED — trajectory list capped at 10,000 points with FIFO eviction.
